module attributes { transform.with_named_sequence } {
  transform.named_sequence @codegen(%arg0: !transform.any_op) {
    ...
    // %15 = transform.lexon.eliminate_gpu_barriers %14 : (!transform.any_op) -> !transform.any_op
    ...
    transform.lexon.hoist_static_alloc %15 : (!transform.any_op) -> ()
    ...
    transform.apply_patterns to %15 {
      transform.apply_patterns.lexon.unroll_vectors_gpu_mma_sync
    } : !transform.any_op
    ...
    // transform.lexon.synchronize_loop %16 : (!transform.op<"scf.for">) -> ()
    ...
    transform.lexon.vector.vector_to_mma_conversion %17 {use_mma_sync} : (!transform.any_op) -> ()

    // %18 = transform.lexon.eliminate_gpu_barriers %17 : (!transform.any_op) -> !transform.any_op
    ...
    // transform.lexon.create_async_groups %17 {use_mma_sync} : (!transform.any_op) -> ()
    transform.apply_patterns to %17 {
      transform.apply_patterns.linalg.tiling_canonicalization
      transform.apply_patterns.scf.for_loop_canonicalization
      transform.apply_patterns.canonicalization
    ...
    // %22 = transform.lexon.pipeline_shared_memory_copies %21 {depth = 3 : i64, use_mma_sync, peel_epilogue} : (!transform.any_op) -> !transform.any_op
