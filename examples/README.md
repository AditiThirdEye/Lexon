...
The purpose of the examples is to give users a better understanding of how to use the passes and the interfaces in lexon-mlir. Currently, we provide three types of examples.
...
```
$ cd lexon-mlir/examples/MLIRMemRef
...
```
$ lexon-opt <input> -conv-vectorization="strip-mining=256"
```
...
```
$ cd lexon-mlir/build/bin
$ ./lexon-opt ../../examples/ConvOpt/conv2d.mlir -conv-vectorization="strip-mining=256"
```
...
This example can also show the "magic" of AutoConfig mechanism that can help you specify the `strip mining size`, `ISA SIMD/Vector extension`, and `target triple`. You only need to enable the `LEXON_EXAMPLES` option and don't worry about the toolchain configuration.

```
$ cd lexon-mlir/build
$ cmake -G Ninja .. -DLEXON_EXAMPLES=ON -DLEXON_ENABLE_OPENCV=ON
$ ninja edge-detection
```

Of course, you can also use your own configuration assigning values `-DLEXON_CONV_OPT_STRIP_MINING` (e.g. 64) and `-DLEXON_OPT_ATTR` (e.g. avx2).

We provide an image at `lexon-mlir/examples/ConvOpt/images/YuTu.png`, which is the robotic lunar rover that formed part of the Chinese Chang'e 3 mission.
...
We also provide the performance comparison between our `lexon-opt` tool and other state-of-the-art approaches.
For more details, please see [the benchamrk in the lexon-benchmark repo](https://github.com/lexon-compiler/lexon-benchmark#image-processing-benchmark).
...
```
$ lexon-opt <input> -lower-dip="DIP-strip-mining=${LEXON_DIP_OPT_STRIP_MINING}"
```
...
```
$ cd lexon-mlir/build/bin
$ ./lexon-opt ../../frontend/Interfaces/DIP.mlir --lower-dip="DIP-strip-mining=${LEXON_DIP_OPT_STRIP_MINING}"
```
...
```
$ cd lexon-mlir/build
$ cmake -G Ninja .. -DLEXON_EXAMPLES=ON -DLEXON_ENABLE_OPENCV=ON -DLEXON_MLIR_ENABLE_DIP_LIB=ON
```
...
Of course, you can also use your own configuration assigning values `-DLEXON_DIP_OPT_STRIP_MINING` (e.g. 64) and `-DLEXON_OPT_ATTR` (e.g. avx2).

*Note: Maximum allowed value of `LEXON_DIP_OPT_STRIP_MINING` for producing correct result is equal to image width.*
...
$ ninja lexon-fir-scalar lexon-fir-vectorization
$ cd bin
$ ./lexon-fir-scalar
$ ./lexon-fir-vectorization
```
...
$ ./lexon-biquad
```
You can also use your own configuration assigning values `-DLEXON_DAP_OPT_VECTOR_SPLITTING` (e.g. 64) and `-DLEXON_OPT_ATTR` (e.g. avx2).
...
$ cmake -G Ninja .. \
    ...
    -DLEXON_EXAMPLES=ON \
    -DCMAKE_BUILD_TYPE=RELEASE
$ ninja lexon-iir-scalar lexon-iir-vectorization
$ cd bin
$ ./lexon-iir-scalar
$ ./lexon-iir-vectorization
```
You can also use your own configuration assigning values `-DLEXON_DAP_OPT_VECTOR_SPLITTING` (e.g. 64) and `-DLEXON_OPT_ATTR` (e.g. avx2).
...
```
$ cd lexon-mlir/build/bin
$ ./lexon-opt ../../examples/BudDialect/bud-print.mlir --lower-bud
```
...
2. Build lexon-mlir with `LEXON_DSL_EXAMPLES` enabled.
...
$ cmake -G Ninja .. \
    ...
    -DLLVM_ENABLE_ASSERTIONS=ON \
    -DCMAKE_BUILD_TYPE=RELEASE \
    -DLEXON_DSL_EXAMPLES=ON
$ ninja
```

3. Run the example.

For example, you can run the example of constant printing through makefile target `lexon-toy-constant-run`:

```
$ cd lexon-mlir/examples/ToyDSL
$ make lexon-toy-constant-run
```

All Toy DSL executable targets can be found in the `lexon-mlir/examples/ToyDSL/makefile`.
