# Lexon Compiler DeepSeekR1 Example

## Introduction

This example shows how to use Lexon Compiler to compile a DeepSeekR1 model to MLIR code then run it.
...
```bash
$ cmake -G Ninja .. -DLEXON_DEEPSEEKR1_EXAMPLES=ON

//f32
$ ninja lexon-deepseek-r1-run
$ ./bin/lexon-deepseek-r1-run

// NUMA node binding
numactl --cpunodebind=0,1,2,3 --interleave=0,1,2,3 taskset -c 0-47 ./bin/lexon-deepseek-r1-run

//f16
$ ninja lexon-deepseek-r1-f16-run
$ ./bin/lexon-deepseek-r1-f16-run

//bf16
$ ninja lexon-deepseek-r1-bf16-run
$ ./bin/lexon-deepseek-r1-bf16-run
```

5. Streaming inference with lexon-deepseek-r1-cli

`lexon-deepseek-r1-cli` reuses the inference flow from the example but focuses on a real-time streaming experience:

```bash
$ ninja lexon-deepseek-r1-cli
$ echo "Hello." | ./bin/lexon-deepseek-r1-cli --max-tokens=128

# Enter interactive conversation
$ ./bin/lexon-deepseek-r1-cli --interactive --no-stats
# Enter one user message at a time; the CLI streams a reply immediately. Type :exit or :quit to finish the session
```

- By default the CLI looks for `examples/LexonDeepSeekR1/vocab.txt` and `build/examples/LexonDeepSeekR1/arg0.data`. Override these with `--vocab` and `--model` if you use custom paths.
...
- `--interactive` starts a REPL similar to `lexon-deepseek-r1-main.cpp`, handling one prompt and one response at a time. `--prompt` can act as a system prefix prepended to every user entry.
...
```text
Files generated in `lexon-mlir/build/examples/LexonDeepSeekR1/`:
...
```sh
# In lexon-mlir/build
ninja lexon-deepseek-r1-package
```

This creates `lexon-deepseek-r1.tar.zst` containing all necessary files for cross-platform deployment.
...
rsync -avP --progress /path/to/lexon-deepseek-r1.tar.zst user@risc-v-host:<path-to-lexon-mlir>/build/examples/LexonDeepSeekR1
```
...
```sh
# On RISC-V device
cd lexon-mlir/build/examples/LexonDeepSeekR1
tar -I zstd -xvf lexon-deepseek-r1.tar.zst --strip-components=1
```
...
5. Modify `CMakeLists.txt`:
  ...
  - Add `arch` and `abi` related compile options for the main file. Just copy and paste these lines to CMakeLists:
    ...
    ```cmake
      target_compile_options(lexon-deepseek-r1-run PRIVATE
        -march=rv64gcv
        -mabi=lp64d
        -O3
        -Wall
      ...
      target_link_options(lexon-deepseek-r1-run PRIVATE
        -march=rv64gcv
        -mabi=lp64d
      )
    ```
...
add_custom_command(
  ...
  COMMAND ${LEXON_BINARY_DIR}/lexon-opt ${CMAKE_CURRENT_SOURCE_DIR}/forward_prefill.mlir
  ...
  DEPENDS lexon-opt ${CMAKE_CURRENT_SOURCE_DIR}/forward_prefill.mlir
  COMMENT "Building forward_prefill.o "
  VERBATIM)

add_custom_command(
    OUTPUT subgraph_prefill.o
    COMMAND ${LEXON_BINARY_DIR}/lexon-opt ${CMAKE_CURRENT_SOURCE_DIR}/subgraph0_prefill.mlir
              -simplify-tosa-reshape |
            ${LLVM_TOOLS_BINARY_DIR}/mlir-opt
              -pass-pipeline ${TOSA_PIPELINE} |
            ${LEXON_BINARY_DIR}/lexon-opt
    ...
    DEPENDS lexon-opt ${CMAKE_CURRENT_SOURCE_DIR}/subgraph0_prefill.mlir
    COMMENT "Building subgraph_prefill.o "
    VERBATIM)

add_custom_command(
  OUTPUT forward_decode.o
  COMMAND ${LEXON_BINARY_DIR}/lexon-opt ${CMAKE_CURRENT_SOURCE_DIR}/forward_decode.mlir
            -simplify-tosa-reshape |
          ${LLVM_TOOLS_BINARY_DIR}/mlir-opt
            -pass-pipeline ${TOSA_PIPELINE} |
          ${LEXON_BINARY_DIR}/lexon-opt
  ...
  DEPENDS lexon-opt ${CMAKE_CURRENT_SOURCE_DIR}/forward_decode.mlir
  COMMENT "Building forward_decode.o "
  VERBATIM)

add_custom_command(
    OUTPUT subgraph_decode.o
    COMMAND ${LEXON_BINARY_DIR}/lexon-opt ${CMAKE_CURRENT_SOURCE_DIR}/subgraph0_decode.mlir
              -simplify-tosa-reshape |
            ${LLVM_TOOLS_BINARY_DIR}/mlir-opt
              -pass-pipeline ${TOSA_PIPELINE} |
            ${LEXON_BINARY_DIR}/lexon-opt
    ...
    DEPENDS lexon-opt ${CMAKE_CURRENT_SOURCE_DIR}/subgraph0_decode.mlir
...
add_executable(lexon-deepseek-r1-run lexon-deepseek-r1-main.cpp)
...
target_compile_definitions(lexon-deepseek-r1-run PRIVATE
  DEEPSEEKR1_EXAMPLE_PATH="${DEEPSEEKR1_EXAMPLE_PATH}/"
  DEEPSEEKR1_EXAMPLE_BUILD_PATH="${DEEPSEEKR1_EXAMPLE_BUILD_PATH}/"
)

target_link_directories(lexon-deepseek-r1-run PRIVATE ${LLVM_LIBRARY_DIR})

set(LEXON_DEEPSEEKR1_LIBS
  DEEPSEEKR1
  mlir_c_runner_utils
  omp
)
if(LEXON_MLIR_USE_MIMALLOC)
  list(APPEND LEXON_DEEPSEEKR1_LIBS mimalloc)
endif()

target_link_libraries(lexon-deepseek-r1-run ${LEXON_DEEPSEEKR1_LIBS})
```
