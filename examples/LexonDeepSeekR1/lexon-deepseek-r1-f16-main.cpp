//===- lexon-deepseek-r1-f16-main.cpp -------------------------------------===//
//
...
void tokenizeInput(const std::string &vocabFile,
  ...
  std::cout << "Vocab file: " << std::filesystem::canonical(vocabFile)
            << std::endl;
  const auto lexonTokenizeStart = std::chrono::high_resolution_clock::now();
  inputContainer.tokenizeDeepSeekR1(vocabFile, MaxTokenLength);
  const auto lexonTokenizeEnd = std::chrono::high_resolution_clock::now();
  const std::chrono::duration<double, std::milli> lexonTokenizeTime =
      lexonTokenizeEnd - lexonTokenizeStart;
  printLogLabel();
  std::cout << "Tokenize time: " << lexonTokenizeTime.count() << "ms"
            << std::endl;
}
...
int main() {
  ...
  const std::string title = "DeepSeekR1 Inference Powered by Lexon Compiler";
