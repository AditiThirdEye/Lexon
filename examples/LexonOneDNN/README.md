# LexonOneDNN: External Operator Library Integration

This example demonstrates how to integrate external operator libraries (oneDNN) into the lexon-mlir framework by replacing specific operators (e.g., MatMul) with optimized external library calls at the Lexon Graph level.
...
- **lexon-mlir:** Built from source
- **PyTorch:** 2.0+

### Build and Run

```bash
# In lexon-mlir root directory
cd build

# Build (automatically executes all compilation stages)
cmake --build . --target lexon-onednn-run

# Run
./bin/lexon-onednn-run
```
...
```
PyTorch → Lexon Graph (MatmulOp) → TOSA → Linalg → Optimized Loops → Machine Code
```

### With External Library
```
PyTorch → Lexon Graph (MatmulOp)
...
5. **CMake Integration**: Fully integrated into lexon-mlir build system
...
Generated in `build/examples/LexonOneDNN/`:
...
- `lexon-onednn-run` - Executable (in `build/bin/`)
...
- [lexon-mlir Project](https://github.com/lexon-compiler/lexon-mlir)
