...
module {
  ...
  func.func @sgemm_scalable_naive(%a : memref<?x?xf32>, %b : memref<?x?xf32>, %c : memref<?x?xf32>) {
    ...
    %vec_len = arith.constant 32 : index
    %vscale = vector.vscale
    %actual_vl = arith.muli %vscale, %vec_len : index
    ...
    scf.parallel (%m_idx) = (%c0) to (%m) step (%unroll) {
      ...
      %n_body_bound_ = arith.subi %n, %actual_vl : index
      %n_body_bound = arith.addi %n_body_bound_, %c1 : index

      %n_iter_idx = scf.for %n_idx = %c0 to %n_body_bound step %actual_vl
          iter_args(%n_iter_idx_init = %c0) -> (index) {
        %sum_init = arith.constant dense<0.> : vector<[32]xf32>
        ...
        %sum_iter_vec_4, %sum_iter_vec_5, %sum_iter_vec_6, %sum_iter_vec_7
            ...
            -> (vector<[32]xf32>, vector<[32]xf32>, vector<[32]xf32>, vector<[32]xf32>,
                vector<[32]xf32>, vector<[32]xf32>, vector<[32]xf32>, vector<[32]xf32>) {
            ...
            %a_vec_0 = vector.broadcast %a_ele_0 : f32 to vector<[32]xf32>
            %a_vec_1 = vector.broadcast %a_ele_1 : f32 to vector<[32]xf32>
            %a_vec_2 = vector.broadcast %a_ele_2 : f32 to vector<[32]xf32>
            %a_vec_3 = vector.broadcast %a_ele_3 : f32 to vector<[32]xf32>
            %a_vec_4 = vector.broadcast %a_ele_4 : f32 to vector<[32]xf32>
            %a_vec_5 = vector.broadcast %a_ele_5 : f32 to vector<[32]xf32>
            %a_vec_6 = vector.broadcast %a_ele_6 : f32 to vector<[32]xf32>
            %a_vec_7 = vector.broadcast %a_ele_7 : f32 to vector<[32]xf32>
            %b_vec = vector.load %b[%k_idx, %n_idx] : memref<?x?xf32>, vector<[32]xf32>
            %res_sum_vec_0 = vector.fma %a_vec_0, %b_vec, %sum_vec_0 : vector<[32]xf32>
            %res_sum_vec_1 = vector.fma %a_vec_1, %b_vec, %sum_vec_1 : vector<[32]xf32>
            %res_sum_vec_2 = vector.fma %a_vec_2, %b_vec, %sum_vec_2 : vector<[32]xf32>
            %res_sum_vec_3 = vector.fma %a_vec_3, %b_vec, %sum_vec_3 : vector<[32]xf32>
            %res_sum_vec_4 = vector.fma %a_vec_4, %b_vec, %sum_vec_4 : vector<[32]xf32>
            %res_sum_vec_5 = vector.fma %a_vec_5, %b_vec, %sum_vec_5 : vector<[32]xf32>
            %res_sum_vec_6 = vector.fma %a_vec_6, %b_vec, %sum_vec_6 : vector<[32]xf32>
            %res_sum_vec_7 = vector.fma %a_vec_7, %b_vec, %sum_vec_7 : vector<[32]xf32>
            scf.yield %res_sum_vec_0, %res_sum_vec_1, %res_sum_vec_2, %res_sum_vec_3,
                        %res_sum_vec_4, %res_sum_vec_5, %res_sum_vec_6, %res_sum_vec_7
                : vector<[32]xf32>, vector<[32]xf32>, vector<[32]xf32>, vector<[32]xf32>,
                vector<[32]xf32>, vector<[32]xf32>, vector<[32]xf32>, vector<[32]xf32>
        }
        vector.store %sum_iter_vec_0, %c[%m_idx, %n_idx] : memref<?x?xf32>, vector<[32]xf32>
        vector.store %sum_iter_vec_1, %c[%m_idx_1, %n_idx] : memref<?x?xf32>, vector<[32]xf32>
        vector.store %sum_iter_vec_2, %c[%m_idx_2, %n_idx] : memref<?x?xf32>, vector<[32]xf32>
        vector.store %sum_iter_vec_3, %c[%m_idx_3, %n_idx] : memref<?x?xf32>, vector<[32]xf32>
        vector.store %sum_iter_vec_4, %c[%m_idx_4, %n_idx] : memref<?x?xf32>, vector<[32]xf32>
        vector.store %sum_iter_vec_5, %c[%m_idx_5, %n_idx] : memref<?x?xf32>, vector<[32]xf32>
        vector.store %sum_iter_vec_6, %c[%m_idx_6, %n_idx] : memref<?x?xf32>, vector<[32]xf32>
        vector.store %sum_iter_vec_7, %c[%m_idx_7, %n_idx] : memref<?x?xf32>, vector<[32]xf32>
        %k_next = arith.addi %n_idx, %actual_vl : index
  ...
  func.func @main(){
    ...
    call @sgemm_scalable_naive(%A, %B, %C) : (memref<?x?xf32>, memref<?x?xf32>, memref<?x?xf32>) -> ()
