...
module {
  ...
  func.func @sgemm_vl_32(%a_aligned : memref<?x?xf32>, %b_aligned : memref<?x?xf32>, %c_aligned : memref<?x?xf32>) {
    ...
    %m = memref.dim %a_aligned, %c0 : memref<?x?xf32>
    %n = memref.dim %c_aligned, %c1 : memref<?x?xf32>
    %k = memref.dim %a_aligned, %c1 : memref<?x?xf32>
    ...
    scf.parallel (%m_idx) = (%c0) to (%m) step (%unroll) {
      ...
      %n_iter_idx = scf.for %n_idx = %c0 to %n_body_bound step %step
        ...
        %sum_iter_vec_4, %sum_iter_vec_5, %sum_iter_vec_6, %sum_iter_vec_7
            ...
            -> (vector<32xf32>, vector<32xf32>, vector<32xf32>, vector<32xf32>,
                vector<32xf32>, vector<32xf32>, vector<32xf32>, vector<32xf32>) {
            %a_ele_0 = memref.load %a_aligned[%m_idx, %k_idx] : memref<?x?xf32>
            %a_ele_1 = memref.load %a_aligned[%m_idx_1, %k_idx] : memref<?x?xf32>
            %a_ele_2 = memref.load %a_aligned[%m_idx_2, %k_idx] : memref<?x?xf32>
            %a_ele_3 = memref.load %a_aligned[%m_idx_3, %k_idx] : memref<?x?xf32>
            %a_ele_4 = memref.load %a_aligned[%m_idx_4, %k_idx] : memref<?x?xf32>
            %a_ele_5 = memref.load %a_aligned[%m_idx_5, %k_idx] : memref<?x?xf32>
            %a_ele_6 = memref.load %a_aligned[%m_idx_6, %k_idx] : memref<?x?xf32>
            %a_ele_7 = memref.load %a_aligned[%m_idx_7, %k_idx] : memref<?x?xf32>
      ...
      scf.for %n_idx = %n_iter_idx to %n step %c1 {
        ...
        %sum_iter_4, %sum_iter_5, %sum_iter_6, %sum_iter_7
          ...
          %b_ele = memref.load %b_aligned[%k_idx, %n_idx] : memref<?x?xf32>
        ...
        memref.store %sum_iter_0, %c_aligned[%m_idx, %n_idx] : memref<?x?xf32>
        memref.store %sum_iter_1, %c_aligned[%m_idx_1, %n_idx] : memref<?x?xf32>
        memref.store %sum_iter_2, %c_aligned[%m_idx_2, %n_idx] : memref<?x?xf32>
        memref.store %sum_iter_3, %c_aligned[%m_idx_3, %n_idx] : memref<?x?xf32>
        memref.store %sum_iter_4, %c_aligned[%m_idx_4, %n_idx] : memref<?x?xf32>
        memref.store %sum_iter_5, %c_aligned[%m_idx_5, %n_idx] : memref<?x?xf32>
        memref.store %sum_iter_6, %c_aligned[%m_idx_6, %n_idx] : memref<?x?xf32>
        memref.store %sum_iter_7, %c_aligned[%m_idx_7, %n_idx] : memref<?x?xf32>
      }
    }
