...
func.func @main(){
  ...
  %m0 = call @alloc_f32(%c1, %c64, %c1, %f2) : (index, index, index, f32) -> memref<?x?x?xf32>
  %m1 = call @alloc_f32(%c1, %c1, %c1, %f3) : (index, index, index, f32) -> memref<?x?x?xf32>
  %m2 = call @alloc_f32(%c1, %c64, %c1, %f0) : (index, index, index, f32) -> memref<?x?x?xf32>

  // CHECK: [6{{(, 6)*}}]
  call @batch_matmul(%m0, %m1, %m2) : (memref<?x?x?xf32>, memref<?x?x?xf32>, memref<?x?x?xf32>) -> ()

  %m3 = call @alloc_f32(%c12, %c1, %c128, %f2) : (index, index, index, f32) -> memref<?x?x?xf32>
  %m4 = call @alloc_f32(%c12, %c128, %c1024, %f3) : (index, index, index, f32) -> memref<?x?x?xf32>
  %m5 = call @alloc_f32(%c12, %c1, %c1024, %f0) : (index, index, index, f32) -> memref<?x?x?xf32>

  // CHECK: [768{{(, 768)*}}]
  call @batch_matmul(%m3, %m4, %m5) : (memref<?x?x?xf32>, memref<?x?x?xf32>, memref<?x?x?xf32>) -> ()

  %m6 = call @alloc_f32(%c12, %c1, %c1024, %f2) : (index, index, index, f32) -> memref<?x?x?xf32>
  %m7 = call @alloc_f32(%c12, %c1024, %c128, %f3) : (index, index, index, f32) -> memref<?x?x?xf32>
  %m8 = call @alloc_f32(%c12, %c1, %c128, %f0) : (index, index, index, f32) -> memref<?x?x?xf32>

  // CHECK: [6144{{(, 6144)*}}]
  call @batch_matmul(%m6, %m7, %m8) : (memref<?x?x?xf32>, memref<?x?x?xf32>, memref<?x?x?xf32>) -> ()
