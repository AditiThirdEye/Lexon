// RUN: lexon-opt  %s -convert-linalg-to-loops -expand-strided-metadata -lower-affine \
...
module {
  ...
  func.func @lexon_batchmatmul_f32(%A: memref<?x?x?xf32>, %B: memref<?x?x?xf32>, %C: memref<?x?x?xf32>) {
    linalg.batch_matmul
  ...
  func.func @main(){
      ...
      call @lexon_batchmatmul_f32(%A, %B, %C) : (memref<?x?x?xf32>, memref<?x?x?xf32>, memref<?x?x?xf32>) -> ()
