...
# Add the parent directory for 'lexon' packages
sys.path.append(
    os.path.abspath(
        os.path.join(
...
from lexon.compiler.graph import GraphDriver
from lexon.compiler.graph.transform import simply_fuse
...
def main():
    ...
    # Save Lexon Graph representation before lowering to TOSA
    print("Saving Lexon Graph representation...")
    with open(os.path.join(output_dir, "graph.log"), "w") as graph_file:
        print("=" * 80, file=graph_file)
        print("LEXON GRAPH REPRESENTATION", file=graph_file)
        print("=" * 80, file=graph_file)
    ...
    with open(os.path.join(output_dir, "graph_fused.log"), "w") as fused_file:
        ...
        print("LEXON GRAPH AFTER FUSION", file=fused_file)
        print("=" * 80, file=fused_file)
        print(
    ...
    print(f"Generated Lexon Graph logs: graph.log and graph_fused.log")
