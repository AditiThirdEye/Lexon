...
public:
  ...
  matchAndRewrite(vector_exp::GetVLOp op, vector_exp::GetVLOp::Adaptor adaptor,
    ...
    auto maxVLOp = rewriter.create<lexon::rvv::RVVSetVlOp>(
...
public:
  ...
  matchAndRewrite(vector_exp::SetVLOp op, vector_exp::SetVLOp::Adaptor adaptor,
    ...
    IndexType indexType = IndexType::get(ctx);
    rewriter.create<lexon::rvv::RVVSetVlOp>(loc, indexType, avlValue, sewVal,
                                            lmulVal);
    ...
    for (Operation &innerOp : block) {
      if (isa<arith::AddIOp>(innerOp)) {
        ...
        Value resultValue = rewriter.create<lexon::rvv::RVVAddOp>(
        ...
        Value resultValue = rewriter.create<lexon::rvv::RVVLoadOp>(
        ...
        rewriter.create<lexon::rvv::RVVStoreOp>(loc, valueToStore, baseOp, idx,
                                                avlValue);
      }
    }
