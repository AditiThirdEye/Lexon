...
static Operation *createGenericElementwiseVIR(Operation *op,
  ...
  auto ensureVec = [&](Value v) -> Value {
    ...
    if (cur && isa<lexon::vir::DynamicVectorType>(cur.getType())) {
      return cur;
    }
    // Broadcast scalar to vector type based on its scalar type.
    Type scalarTy = cur.getType();
    auto vecTy = lexon::vir::DynamicVectorType::get(virShape, scalarTy);
    return rewriter.create<lexon::vir::BroadcastOp>(loc, vecTy, cur);
  };
  ...
  for (Type resTy : op->getResultTypes()) {
    ...
    resultTypes.push_back(lexon::vir::DynamicVectorType::get(virShape, resTy));
  }
...
static lexon::vir::SetVLOp createSetVLRegion(PatternRewriter &rewriter,
                                             Location loc, Value vlVal) {
  auto setVl = rewriter.create<lexon::vir::SetVLOp>(
...
static LogicalResult mapInputsToVIRVectors(linalg::LinalgOp linalgOp,
  ...
  for (OpOperand *opOperand : linalgOp.getOpOperandsMatchingBBargs()) {
    ...
    auto vecTy =
        lexon::vir::DynamicVectorType::get(virShape, memrefTy.getElementType());
    auto loaded =
        rewriter.create<lexon::vir::LoadOp>(loc, vecTy, base, zeroIdx);
...
static LogicalResult convertBodyToVIR(linalg::LinalgOp linalgOp,
  ...
  for (Operation &inner : linalgOp.getBlock()->getOperations()) {
    ...
    if (auto cst = dyn_cast<arith::ConstantOp>(inner)) {
      ...
      auto vecTy = lexon::vir::DynamicVectorType::get(virShape, cst.getType());
      auto v =
          rewriter.create<lexon::vir::BroadcastOp>(loc, vecTy, cst.getResult());
...
static LogicalResult storeYieldValues(linalg::LinalgOp linalgOp,
  ...
  for (auto [i, yv] : llvm::enumerate(yield.getOperands())) {
    ...
    if (!mapped && valueMap.contains(yv)) {
      ...
      auto outVecTy = lexon::vir::DynamicVectorType::get(virShape, outElemTy);
      mapped = rewriter.create<lexon::vir::BroadcastOp>(loc, outVecTy, scalar);
    }
    auto initOpd = linalgOp.getDpsInitOperand(i);
    ...
    rewriter.create<lexon::vir::StoreOp>(loc, mapped, base, zeroIdx);
  }
  return success();
}
...
public:
  ...
  void getDependentDialects(DialectRegistry &registry) const override {
    registry.insert<arith::ArithDialect, linalg::LinalgDialect,
                    memref::MemRefDialect, lexon::vir::VIRDialect,
