...
lexon::gpu::TransformExtensions::TransformExtensions() {
...
void lexon::registerLexonGPUTransformOps(DialectRegistry &registry) {
  registry.addExtensions<lexon::gpu::TransformExtensions>();
}
...
DiagnosedSilenceableFailure lexon::gpu::HoistStaticAllocOp::applyToOne(
...
void lexon::gpu::HoistStaticAllocOp::getEffects(
...
getGPUTensorCoreNativeMmaSyncVectorSize(Operation *op) {
  return lexon::gpu::getMmaNativeVectorSize(op);
}

void lexon::gpu::ApplyUnrollVectorsGpuMmaSyncPatternsOp::populatePatterns(
...
void lexon::gpu::VectorToMMAConversionOp::getEffects(
    SmallVectorImpl<MemoryEffects::EffectInstance> &effects) {
  mlir::transform::onlyReadsHandle(getTargetMutable(), effects);
  mlir::transform::modifiesPayload(effects);
}

DiagnosedSilenceableFailure lexon::gpu::VectorToMMAConversionOp::applyToOne(
