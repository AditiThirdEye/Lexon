...
#include "lexon-mlir-c/RegisterEverything.h"
...
namespace lexon {
void registerLexonToLLVMIRTranslation();
}

void lexonRegisterAllDialects(MlirDialectRegistry registry) {
  ...
  // Register all Dialects from LEXON MLIR
  mlir::lexon::registerAllDialects(*unwrap(registry));
}

void lexonRegisterAllTranslations(MlirContext context) {
  ...
  // Register all Translations from LEXON MLIR
...
void lexonRegisterAllPasses() {
  // Register all Passes from UPSTREAM MLIR
  mlir::registerAllPasses();

  // Register all Passes from LEXON MLIR
  mlir::lexon::registerAllPasses();
}
