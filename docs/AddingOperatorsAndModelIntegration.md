## Guide: Adding Operators and Integrating Models in Lexon-MLIR

This guide targets developers who want to add frontend operator mappings (PyTorch → Lexon Graph → MLIR) and import/integrate models in Lexon-MLIR. It is based on the repository directories `examples` and `frontend/Python`. After reading, you will be able to:

- Add a Lexon Graph-level operator and lower it to MLIR (TOSA/Linalg/Math, etc.).
- Import PyTorch functions/models via TorchDynamo or the Export API into Lexon Graph and MLIR modules, and obtain/package parameters.

### Relevant Directory Overview

- `frontend/Python/frontend.py`
  - Frontend entry `DynamoCompiler`: imports FX Graph → Lexon Graph, assembles registries, and wraps execution/compilation.
- `frontend/Python/graph/`
  - `operation.py`: Lexon Graph operator classes (op types, metadata, etc.).
...
- `frontend/Python/ops/`
  - `tosa.py` / `linalg.py` / `math.py` / `func.py`: Lowering from Lexon Graph ops to MLIR dialect ops, registered via `ops_registry`.
  - `utils.py`: Helpers for types/attributes.
- `examples/LexonPython/`
...
Lexon's Python frontend depends on MLIR Python bindings. Build highlights (see `examples/LexonPython/README.md` and `examples/MLIRPython/README.md`):

1) Build MLIR Python bindings under `lexon-mlir/llvm` and verify with `ninja check-mlir`.
...
## 2. Adding an Operator (Torch → Lexon Graph → MLIR)

Adding a new operator typically involves three layers:

1) Define the operator at the Lexon Graph layer (if a suitable class does not already exist).
2) Implement the lowering function to MLIR (TOSA/Linalg/Math, etc.) and register it into `ops_registry`.
3) Map the Torch Aten/Prims symbol to the Lexon Graph operator class in the frontend import map (`DynamoCompiler._ops_map`).

### Step 1: Define the Operator in Lexon Graph
...
In the target dialect file (e.g., `frontend/Python/ops/tosa.py`), implement the conversion from the Lexon op to the MLIR op:
...
### Step 3: Map Torch Operator to Lexon Operator
...
Notes: Keys are the FX/Aten symbols emitted by Torch (inspectable by running the import flow and printing `gm.graph.print_tabular()`); values are Lexon Graph operator classes.
...
See `examples/LexonPython/module_gen.py`:

```python
import torch
from torch._inductor.decomposition import decompositions as inductor_decomp
from lexon.compiler.frontend import DynamoCompiler
from lexon.compiler.ops import tosa
...
Example: `examples/LexonPython/bert.py`:
...
- For complex models, check `examples/LexonLlama/`, `examples/LexonResNet18/`, etc.
...
- How to find the Torch → Lexon mapping key?
...
- For the new Lexon Graph operator class, is `OpType` set appropriately?
