...
This document uses `BatchMatMul` as an example to demonstrate the complete process of how to add an optimization for a particular operator to the midrange in a lexon project.
...
3. Add build rules and validation data to ensure check-lexon passes checks correctly.
...
4. Add build rules to makefile for building source mlir files(linalg-batchmatmul-f32.mlir).
   ...
   linalg-batchmatmul-f32-lower:
   	@${LEXON_OPT} ./linalg-batchmatmul-f32.mlir \
...
For the full source code, please check [batchmatmul-f32.mlir](../examples/LexonMatmul/linalg-batchmatmul-f32.mlir) and [makefile](../examples/LexonMatmul/makefile).
...
For the full source code, please check [batchmatmul-vectorization.mlir](../examples/LexonMatmul/batchmatmul-vectorization.mlir) and [makefile](../examples/LexonMatmul/makefile)
...
3. Register implemented passes in lexon-opt.
...
For the full source code, please check [BatchMatMulOptimize.cpp](../midend/lib/Conversion/MatMulOptimization/BatchMatMulOptimize.cpp), [CMakeLists.txt](../midend/lib/Conversion/MatMulOptimization/CMakeLists.txt) and [lexon-opt.cpp](../tools/lexon-opt/lexon-opt.cpp)
...
1. In CMakeLists.txt, add a custom compilation command to generate the target file (`add_custom_command`), compile the MLIR source code, and encapsulate the generated arithmetic as a static library (`add_library`), and link to the executable target afterwards.
   ...
   add_custom_command(OUTPUT batch_matmul_vectorization.o
     COMMAND cat ${LEXON_SOURCE_DIR}/benchmarks/DeepLearning/Ops/BatchMatMulOp/BatchMatMul.mlir |
             sed 's/@batch_matmul/@batch_matmul_vectorization/' |
             ${LEXON_MLIR_BUILD_DIR}/bin/lexon-opt
             -batchmatmul-optimize
             ...
             -convert-func-to-llvm 
             -reconcile-unrealized-casts|
             ${LEXON_MLIR_BUILD_DIR}/bin/lexon-translate --lexon-to-llvmir -o batch_matmul_vectorization.ll
   ...
   add_custom_command(OUTPUT batch_matmul_vectorization0.o
     COMMAND cat ${LEXON_SOURCE_DIR}/benchmarks/DeepLearning/Ops/BatchMatMulOp/BatchMatMulVec.mlir |
             ...
             -convert-func-to-llvm 
             -reconcile-unrealized-casts|
             ${LEXON_MLIR_BUILD_DIR}/bin/lexon-translate --lexon-to-llvmir -o batch_matmul_vectorization0.ll
