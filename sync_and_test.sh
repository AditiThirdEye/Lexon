...
if [ ! -f "CMakeLists.txt" ] || [ ! -d "frontend/Python" ]; then
    print_error "Please run this script in the lexon-mlir root directory"
    exit 1
fi

# Set paths
LEXON_ROOT=$(pwd)
BUILD_DIR="$LEXON_ROOT/build"
PYTHON_PACKAGES_DIR="$BUILD_DIR/python_packages"
FRONTEND_PYTHON_DIR="$LEXON_ROOT/frontend/Python"
...
if [ ! -d "$PYTHON_PACKAGES_DIR" ]; then
    print_error "python_packages directory does not exist, please ensure LEXON_MLIR_ENABLE_PYTHON_PACKAGES is enabled"
    exit 1
fi
...
print_info "Syncing frontend/Python files to build/python_packages/lexon/compiler/"
sync_python_files "$FRONTEND_PYTHON_DIR" "$PYTHON_PACKAGES_DIR/lexon/compiler"
...
export LEXON_MLIR_BUILD_DIR="$BUILD_DIR"
export LLVM_MLIR_BUILD_DIR="$LEXON_ROOT/llvm/build"
...
print_info "LEXON_MLIR_BUILD_DIR=$LEXON_MLIR_BUILD_DIR"
