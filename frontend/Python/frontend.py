...
# ===---------------------------------------------------------------------------
#
# This is the entry of the Lexon Compiler frontend.
#
...
class DynamoCompiler:
    """
    Dynamo Compiler is one of the frontends of Lexon Compiler.
    Dynamo Compiler acts as a custom compiler for the TorchDynamo framework,
    which converts an FX Graph into an equivalent Lexon Graph and MLIR module.
    ...
    def __init__(
        ...
        Attributes:
            ...
            _imported_graphs: The lexon graphs from dynamo importer.
            _ops_registry (dict, optional): The lexon operations' lower func
            registry.
            _imported_params: The model params extract from torch.
            _ops_map: The torch aten ops map with lexon ops.
    ...
    def imported_graphs(self):
        """Returns the imported lexon graphs after compilation."""
    ...
    def _create_node(
        ...
        """
        Create lexon op node from torch aten op.

        Args:
            gm_node_name: The op node class map to lexon op by _ops_map.
        ...
        lexon_node = op_class()
        lexon_node._name = node_name
        if gm_node_name == "output":
            for input_arg in node_input[0]:
                lexon_node.add_argument(str(input_arg))
            return lexon_node
        for input_arg in node_input:
            if isinstance(input_arg, torch.fx.Node):
                lexon_node.add_argument(str(input_arg))
                lexon_node.add_parent(str(input_arg))
            elif isinstance(input_arg, torch.dtype):
                lexon_node.add_argument(
                    self._torch_dtype_translate(str(input_arg))
                )
            else:
                lexon_node.add_argument(input_arg)
        for user in node_users:
            lexon_node.add_children(user)
        if node_kwargs is None:
            node_kwargs = {}
        lexon_node._keyword_arguments.update(node_kwargs)
        lexon_node._tensor_meta["shape"] = node_output_shape
        lexon_node._tensor_meta["dtype"] = node_output_dtype
        return lexon_node
    ...
    def _compile_fx(
        ...
        """
        Compiles the provided FX Graph to Lexon Graph.
    ...
    def importer(self, model, *args, **kwargs) -> List[Graph]:
        ...
        Returns:
            imported_graphs: The imported lexon graphs.
        """
    ...
    def dynamo_run(self):
        """
        A callable method that wraps around the `exec_lexon_graph` method.

        Returns:
            exec_lexon_graph: The function of the ahead-of-time compiled module,
            return for torchdynamo's call.
        """
