...
# ===---------------------------------------------------------------------------
#
# This is the graph level of the Lexon Compiler frontend.
#
# ===---------------------------------------------------------------------------
...
class Graph:
    """
    Graph is a graph-level expression for the Lexon Compiler frontends.
    ...
    def compile(self):
        """
        Compile graph from Lexon Graph to LLVM IR.
        """
        self.lower_to_top_level_ir()
...
class GraphImporter:
    """
    Imports an lexon graph and generates an MLIR module in high-level dialects.
    ...
    def __init__(
        ...
        """
        Initializes the lexon Graph importer.

        Args:
            gm (Graph): The lexon graph that will be imported.
            inputs (List[TensorMeta]): Input tensor(s) of the lexon graph.
    ...
    def import_graph(self) -> ir.Module:
        """
        Imports lexon graph and generates an MLIR module in high-level dialects.
    ...
    def import_main_graph(self) -> ir.Module:
        """
        Imports lexon main graph to organize all subgraphs and generates an MLIR
    ...
    def _import_placeholder(
        ...
        """
        Imports a placeholder node from the Lexon graph.
    ...
    def _import_op(self, node: Op):
        """
        Imports an operation node from the lexon graph.

        Args:
            node (Op): The lexon node representing the operation.
