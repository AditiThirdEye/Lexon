...
# ===---------------------------------------------------------------------------
#
# The registry of mappings from Lexon Graph to MLIR linalg dialect operations.
#
# ===---------------------------------------------------------------------------
...
def add_op(node: AddOp, symbol_table: Dict[Tuple[str, int], ir.Operation]):
    ...
    From lexon AddOp to MLIR arith `constant` operation.
...
def arange_op(
    ...
    From lexon ArangeOp to MLIR arith `constant` operation.
...
def unsqueeze_op(
    ...
    From lexon UnsqueezeOp to MLIR TOSA `reshape` operation.
...
def view_op(
    ...
    From lexon ViewOp to MLIR TOSA `reshape` operation.
...
def embedding_op(
    ...
    From lexon EmbeddingOp to MLIR linalg `generic` operation.
...
def ones_op(
    ...
    From lexon OnesOp to MLIR arith `constant` operation.
...
def full_op(
    ...
    From lexon FullOp to MLIR arith `constant` operation.
...
def lt_op(
    ...
    From lexon LessThanOp to MLIR arith `constant` operation.
...
def masked_fill_op(
    ...
    From lexon MaskedFillOp to MLIR linalg `generic` operation.
...
def slice_op(
    ...
    From lexon SliceOp to MLIR tensor `extract_slice` operation.
...
def expand_op(
    ...
    From lexon ExpandOp to MLIR tensor `extract_slice` operation.
...
def to_copy_op(
    ...
    From lexon ToCopyOp to MLIR linalg `generic`
    operation.
...
def rsub_op(
    ...
    From lexon RsubOp to MLIR linalg `generic` operation.
...
def pow_op(
    ...
    """
    Import the tensor copy operation.
    From lexon PowOp to MLIR linalg `generic`
    operation.
...
def mean_op(
    ...
    """
    Import the tensor copy operation.
    From lexon MeanOp to MLIR linalg `generic` operation.
...
def rsqrt_op(
    ...
    From lexon RsqrtOp to MLIR linalg `generic` operation.
...
def mul_op(
    ...
    From lexon MulOp to MLIR linalg `generic` operation.
...
def t_op(
    ...
    From lexon TransposeOp to MLIR linalg `generic` operation.
...
def matmul_op(
    ...
    From Lexon MatmulOp to MLIR linalg `matmul` operation.
...
def transpose_op(
    ...
    From lexon TransposeSpecificDimOp to MLIR linalg `generic`
    operation.
...
def index_op(
    ...
    From lexon IndexOp to MLIR linalg `generic`
    operation.
...
def neg_op(
    ...
    From lexon NegOp to MLIR linalg `negf` operation.
...
def cat_op(
    ...
    From lexon CatOp to MLIR tensor `insert_slice`
    operation.
...
def squeeze_op(
    ...
    From lexon SqueezeOp to MLIR linalg `generic` operation.
...
def batch_matmul_op(
    ...
    From lexon BatchMatmulOp to MLIR linalg `batch_matmul`
    operation.
...
def div_op(
    ...
    From lexon DivOp to MLIR linalg `generic` operation.
...
def softmax_op(
    ...
    From lexon SoftmaxOp to MLIR linalg `generic` operation.
...
def clone_op(
    ...
    From lexon CloneOp to MLIR tensor `extract_slice`
    operation.
...
def silu_op(
    ...
    From Lexon SiluOp to MLIR linalg `generic` operation.
...
def where_op(
    ...
    From Lexon WhereOp to MLIR linalg `generic` operation.
...
def scalar_tensor_op(node: ScalarTensorOp, symbol_table):
    ...
    From Lexon ScalarTensorOp to MLIR arith `ConstantOp` operation.
    """
    assert len(node.args) == 1
    dtype = node.tensor_meta["dtype"]
...
def ge_op(
    ...
    From lexon GreaterEqualOp to MLIR arith `constant` operation.
    Note: This op, campare two input nodes, and output bool tensor to represent
    compare result.
    Args:
        node: Containing information from the input graph node.
...
def greater_than_op(
    ...
    From lexon GreaterThanOp to MLIR arith `constant` operation.
    Note: This op, campare two input nodes, and output bool tensor to represent
    compare result.
    Args:
        node: Containing information from the input graph node.
...
def unsafe_index_op(
    ...
    From lexon UnsafeIndexOp to MLIR linalg `generic`
    operation.
    Note: This op, get input node slice result by input index.
    Args:
        node: Containing information from the input graph node.
...
def equal_op(
    ...
    """
    Converts a Lexon EqualOp operation to an MLIR comparison operation (CmpIOp or CmpFOp).
    ...
    Parameters:
        node (EqualOp): The Lexon EqualOp node containing the operation details and tensor metadata.
...
def copy_op(node: CopyOp, symbol_table):
    """
    Import the tensor copy operation.
    Converts Lexon CopyOp to an equivalent MLIR linalg.generic operation.
...
def index_put_op(
    ...
    """
    Converts a Lexon IndexPutOp operation to an MLIR operation using scf.ForOp loops.
    ...
    Parameters:
        node (IndexPutOp): The Lexon IndexPutOp containing tensor metadata and index data.
...
def ne_scalar_op(
    ...
    """
    Converts a Lexon NeScalarOp operation to an MLIR comparison operation (CmpIOp or CmpFOp).
    ...
    Parameters:
        node (NeScalarOp): The Lexon NeScalarOp node containing the operation details and tensor metadata.
        symbol_table (dict): A dictionary mapping tensor names to their corresponding MLIR operations.
...
def cumsum_op(
    ...
    """
    Converts a Lexon CumsumOp operation to an MLIR operation using scf.ForOp loops.
    ...
    Parameters:
        node (CumsumOp): The Lexon CumsumOp node containing the operation details and tensor metadata.
        symbol_table (dict): A dictionary mapping tensor names to their corresponding MLIR operations.
...
def tensor_constant_op(
    ...
    """
    Converts a Lexon Constant0Op operation to an MLIR arith.ConstantOp.
    ...
    Parameters:
        node (Constant0Op): The Lexon Constant0Op node containing the tensor shape and data type metadata.
        symbol_table (dict): A dictionary mapping tensor names to their corresponding MLIR operations.
...
def lift_fresh_copy_op(
    ...
    """
    Converts a Lexon LiftFreshCopyOp operation to an MLIR tosa.IdentityOp.
    ...
    Parameters:
        node (LiftFreshCopyOp): The Lexon LiftFreshCopyOp node containing the operation details.
        symbol_table (dict): A dictionary mapping tensor names to their corresponding MLIR operations.
...
def repeat_op(
    ...
    """
    Converts a Lexon RepeatOp operation to an MLIR operation.
    ...
    Parameters:
        node (RepeatOp): The Lexon RepeatOp node containing the tensor and repeat factors.
        symbol_table (dict): A dictionary mapping tensor names to their corresponding MLIR operations.
...
def as_strided_op(
    ...
    """
    Converts a Lexon AsStridedOp operation to an MLIR operation.
    ...
    Parameters:
        node (AsStridedOp): The Lexon AsStridedOp node containing the tensor and metadata.
        symbol_table (dict): A dictionary mapping tensor names to their corresponding MLIR operations.
