...
# ===---------------------------------------------------------------------------
#
# The registry of mappings from Lexon Graph to MLIR tosa dialect operations.
#
# ===---------------------------------------------------------------------------
...
def addmm_op(
    ...
    From lexon graph ir's `AddMMOp` operator to MLIR linalg `matmul` operation.
...
def bmm_op(node: BatchMatmulOp, symbol_table) -> ir.Operation:
    ...
    From lexon graph ir's `BatchMatmulOp` operator to MLIR TOSA `matmul`
    operation.
    """
    input_ = symbol_table.get((str(node.args[0]), 0))
...
def add_op(node: AddOp, symbol_table):
    ...
    From lexon graph ir's `AddOp` operator to MLIR TOSA `add` operation.
    """
...
def sub_op(node: SubOp, symbol_table):
    ...
    From lexon graph ir's `SubOp` operator to MLIR TOSA `sub` operation.
    """
    input1 = symbol_table.get((str(node.args[0]), 0), node.args[0])
    input2 = symbol_table.get((str(node.args[1]), 0), node.args[1])
...
def mul_op(node: MulOp, symbol_table):
    ...
    From lexon graph ir's `DivOp` operator to MLIR TOSA `div` operation.
    """
...
def tanh_op(node: TanhOp, symbol_table):
    ...
    From lexon graph ir's `TanhOp` operator to MLIR TOSA `tanh` operation.
    """
...
def exp_op(node: ExpOp, symbol_table):
    ...
    From lexon graph ir's `ExpOp` operator to MLIR TOSA `exp` operation.
    """
    input1 = symbol_table.get((str(node.args[0]), 0))
    sizes = ir.RankedTensorType(input1.type).shape
    result_element_type = ir.RankedTensorType(input1.type).element_type
...
def rsqrt_op(node: RsqrtOp, symbol_table):
    ...
    From lexon graph ir's `RsqrtOp` operator to MLIR TOSA `rsqrt` operation.
    """
    input1 = symbol_table.get((str(node.args[0]), 0))
    sizes = ir.RankedTensorType(input1.type).shape
    result_element_type = ir.RankedTensorType(input1.type).element_type
...
def amax_op(node: AmaxOp, symbol_table):
    ...
    From lexon graph ir's `AmaxOp` operator to MLIR TOSA `reduce_max`
    operation.
    """
    input1 = symbol_table.get((str(node.args[0]), 0))
...
def reshape_op(node: ReshapeOp, symbol_table):
    ...
    From lexon graph ir's `ReshapeOp` operator to MLIR TOSA `reshape`
    operation.
...
def unsqueeze_op(node: UnsqueezeOp, symbol_table):
    ...
    From lexon graph ir's `UnsqueezeOp` operator to MLIR TOSA `reshape`
    operation.
    """
...
def select_op(node: SelectOp, symbol_table):
    ...
    From lexon graph ir's `SelectOp` operator to MLIR TOSA `reshape`
    operation.
    """
    input_tensor = symbol_table.get((str(node.args[0]), 0))
    dim = node.args[1]
...
def slice_op(node: SliceOp, symbol_table):
    ...
    From lexon graph ir's `SliceOp` operator to MLIR TOSA `extract_slice`
    operation.
    """
    input_tensor = symbol_table.get((str(node.args[0]), 0))
    dim = node.args[1]
...
def convert_element_type_op(node: ConvertElementTypeOp, symbol_table):
    ...
    From lexon graph ir's `ConvertElementTypeOp` operator to MLIR TOSA
    `cast` operation.
    """
    # maintain a mapping of lexon dtype to mlir types
    types_mapping = {
...
def clone_op(node: CloneOp, symbol_table):
    ...
    From lexon graph ir's `CloneOp` operator to MLIR TOSA `identity`
    operation.
...
def var_mean_op(node: VarMeanOp, symbol_table):
    ...
    From lexon graph ir's `VarMeanOp` operator to two MLIR TOSA `mul`
    operation.
...
def permute_op(node: PermuteOp, symbol_table):
    ...
    From lexon graph ir's `PermuteOp` operator to MLIR TOSA `transpose`
    operation.
    """
    input_tensor = symbol_table.get((str(node.args[0]), 0))
    perm = node.args[1]
...
def embedding_op(node: EmbeddingOp, symbol_table):
    ...
    From lexon graph ir's `EmbeddingOp` operator to MLIR TOSA `reshape`
    operation.
...
def expand_op(node: ExpandOp, symbol_table) -> ir.Operation:
    ...
    From lexon graph ir's `ExpandOp` operator to MLIR TOSA `add` operation.
...
def sum_op(node: SumDimOp, symbol_table):
    ...
    From lexon graph ir's `SumDimOp` operator to MLIR TOSA `reduce_sum`
    """
    input_tensor = symbol_table.get((str(node.args[0]), 0))
...
def t_op(node: TOp, symbol_table):
    ...
    From lexon graph ir's `TOp` operator to MLIR TOSA `transpose` operation
    """
...
def transpose_op(node: TransposeOp, symbol_table):
    ...
    From lexon graph ir's `TransposeOp` operator to MLIR TOSA `transpose`
    operation.
    """
...
def maxpool2d_op(node: MaxPool2dOp, symbol_table):
    ...
    From Lexon MaxPool2dOp to MLIR TOSA `max_pool2d` operation.
    """
    if len(node.args) == 5:
...
def convolution2d_op(node: Conv2dOp, symbol_table):
    ...
    From Lexon Conv2dOp to MLIR TOSA `conv2d` operation.
    arg[0]: Tensor input
    arg[1]: Tensor weight
    arg[2]: Tensor? bias
    arg[3]: SymInt[] stride
...
def relu_op(node: ReluOp, symbol_table):
    ...
    From Lexon ReluOp to MLIR TOSA `maximum` operation.
    """
    assert len(node.args) == 1
    input1 = symbol_table.get((str(node.args[0]), 0))
    if input1 is None:
...
def iota_op(node: IotaOp, symbol_table):
    ...
    From Lexon IotaOp to MLIR TOSA `ConstOp` operation.
    """
    assert len(node.args) == 1
    output_shape = list(node.tensor_meta["shape"])
    dtype = node.tensor_meta["dtype"]
...
def sigmoid_op(node: SigmoidOp, symbol_table):
    ...
    From Lexon SigmoidOp to MLIR TOSA `SigmoidOp` operation.
    """
    assert len(node.args) == 1
    input1 = symbol_table.get((str(node.args[0]), 0))
    if input1 is None:
...
def reciprocal_op(node: ReciprocalOp, symbol_table):
    """
    Import the lexon ReciprocalOp.
    From Lexon ReciprocalOp to MLIR TOSA `ReciprocalOp` operation.
    """
    input_tensor = symbol_table.get((str(node.args[0]), 0))
...
def mean_op(node: MeanOp, symbol_table):
    """
    Import the lexon MeanOp.
    From Lexon MeanOp to MLIR TOSA operation.
    """
