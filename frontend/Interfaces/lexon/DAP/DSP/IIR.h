...
#ifndef FRONTEND_INTERFACES_LEXON_DAP_DSP_IIR
#define FRONTEND_INTERFACES_LEXON_DAP_DSP_IIR
...
void _mlir_ciface_lexon_iir(MemRef<float, 1> *inputLexonConv1D,
                            MemRef<float, 2> *kernelLexonConv1D,
                            MemRef<float, 1> *outputLexonConv1D);

void _mlir_ciface_lexon_iir_vectorization(MemRef<float, 1> *inputLexonConv1D,
                                          MemRef<float, 2> *kernelLexonConv1D,
                                          MemRef<float, 1> *outputLexonConv1D);
}
} // namespace detail
...
void IIR(MemRef<float, N> *input, MemRef<T, 2> *filter,
  ...
  if (!isVectorization)
    detail::_mlir_ciface_lexon_iir(input, filter, output);
  else
    detail::_mlir_ciface_lexon_iir_vectorization(input, filter, output);
}

} // namespace dap

#endif // FRONTEND_INTERFACES_LEXON_DAP_DSP_IIR
