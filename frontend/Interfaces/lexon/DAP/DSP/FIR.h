...
#ifndef FRONTEND_INTERFACES_LEXON_DAP_DSP_FIR
#define FRONTEND_INTERFACES_LEXON_DAP_DSP_FIR

#include "lexon/Core/Container.h"
#include "lexon/DAP/AudioContainer.h"
#include "lexon/DAP/DSP/Window.h"
...
void _mlir_ciface_lexon_fir(MemRef<float, 1> *inputLexonFIR,
                            MemRef<float, 1> *kernelLexonFIR,
                            MemRef<float, 1> *outputLexonFIR);
void _mlir_ciface_lexon_fir_vectorization(MemRef<float, 1> *inputLexonFIR,
                                          MemRef<float, 1> *kernelLexonFIR,
                                          MemRef<float, 1> *outputLexonFIR);
}
} // namespace detail
...
void FIR(MemRef<float, N> *input, MemRef<T, N> *filter,
  ...
  if (!isVectorization)
    detail::_mlir_ciface_lexon_fir(input, filter, output);
  else
    detail::_mlir_ciface_lexon_fir_vectorization(input, filter, output);
}
} // namespace dap

#endif // FRONTEND_INTERFACES_LEXON_DAP_DSP_FIR
